...
if __name__ == "__main__":
    X, y, feature_names = parse_biomed_data_to_ndarray("wdbc_new", return_X_y=True)
    subsamples = subsample_features(
        X, feature_names, 4, all_possible_combinations=False
    )
    ...
    for X_sub, subsampled_features in tqdm(subsamples):
        ...
        for train_idx, test_idx in rs.split(X_sub, y):
    ...
    # for i, (sub_X, sub_features) in enumerate(subsamples):
    #     print(f"Subsample {i+1}:")
    #     print("Selected Features:", sub_features)
    #     print("Subsampled Data Shape:", sub_X.shape)
    #     print()
