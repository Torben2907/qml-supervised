...
def ad_hoc_data(
    ...
    **References:**
    ...
    if n == 2:
        ...
        raise ValueError(
            f"Supported values of 'n' are 2 and 3 only, but {n} is provided."
        )
    ...
    z_i = np.array(
        [reduce(np.kron, [i_2] * i + [z] + [i_2] * (n - i - 1)) for i in range(n)]
    )
    ...
    for x in it.product(*[xvals] * n):
        ...
        phi += sum(
            (
                (np.pi - x_arr[i1]) * (np.pi - x_arr[i2]) * z_i[i1] @ z_i[i2]
    ...
    x_sample, y_sample = _sample_ad_hoc_data(
        sample_total, xvals, training_size + test_size, n
    )

    training_input = {
        key: (x_sample[y_sample == k, :])[:training_size]
